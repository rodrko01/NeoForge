...
public class GrowthChamberMenu extends AbstractContainerMenu {
    ...
    public GrowthChamberMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(2)); // stores progress data not item slots
    }
    ...
    public GrowthChamberMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        ...
        this.addSlot(new SlotItemHandler(blockEntity.itemHandler, 0, 34, 34)); // inputs
        this.addSlot(new SlotItemHandler(blockEntity.itemHandler, 1, 54, 34));
        this.addSlot(new SlotItemHandler(blockEntity.itemHandler, 2, 74, 34));
        this.addSlot(new SlotItemHandler(blockEntity.itemHandler, 3, 94, 34));
        this.addSlot(new SlotItemHandler(blockEntity.itemHandler, 4, 130, 34)); // output
    ...
    public @NotNull ItemStack quickMoveStack(@NotNull Player playerIn, int pIndex) {
        ...
        if (pIndex < PLAYER_INVENTORY_FIRST_SLOT_INDEX) {
            // move from TE to player inv (requires true)
            if (!moveItemStackTo(sourceStack, PLAYER_INVENTORY_FIRST_SLOT_INDEX, PLAYER_INVENTORY_FIRST_SLOT_INDEX + PLAYER_INVENTORY_SLOT_COUNT, true)) { // input slots only
                return ItemStack.EMPTY;  // EMPTY_ITEM
            }
        } else if (pIndex < PLAYER_INVENTORY_FIRST_SLOT_INDEX + PLAYER_INVENTORY_SLOT_COUNT) {
            // move from player inv to TE (requires false)
